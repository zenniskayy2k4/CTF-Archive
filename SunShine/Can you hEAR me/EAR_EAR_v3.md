...
If the MMU enabled bit is set, however, things get a bit more complicated. The remaining 15 bits are shifted left by one (to form a 16-bit value whose LSB is 0) and then used as the physical page number where the page tables to use are located. The page number of the virtual address indexes into this page table to find the 16-bit physical page number for the backing physical page. Then, the low 8 bits of the virtual address are used as the offset into this physical page, thus finally building a 24-bit EAR full. This EAR full is then used to access the physical memory bus.

...
# --- CAN IDs ĐÃ SỬA LẠI ---
CAN_ID_THROTTLE = 0x0102
CAN_ID_STEER = 0x0202
CAN_ID_START_RACE = 0x0402
CAN_ID_BRAKE = 0x0302
CAN_ID_RESET = 0x0502
...
def send_can_frame(net, can_id, data=b''):
    if not net.alive: return
    try:
        frame_bytes = can_id.to_bytes(2, 'little') + data
        frame_hex = frame_bytes.hex()
        payload = json.dumps({"t": "can", "frame": frame_hex})
        net.sock.sendall((payload + '\n').encode('utf-8'))
...
def main():
    ...
    while True:
        ...
        if autopilot_enabled and latest and latest.get('status') == 'ok':
            send_can_frame(net, CAN_ID_THROTTLE, (100).to_bytes(1, byteorder='little', signed=True))
            ...
            if obstacles:
                for ob in obstacles:
                    try:
                        dy = float(ob["dy"])
                        if 0 < dy < 150 and dy < min_dy:
                            min_dy = dy
                            closest_obstacle = ob
                    except (KeyError, ValueError, TypeError):
                        continue
            if closest_obstacle:
                obs_x = float(closest_obstacle["x"])
                obs_w = float(closest_obstacle["w"])
                obs_left_edge = obs_x - obs_w / 2
                obs_right_edge = obs_x + obs_w / 2
                if obs_left_edge - 5 < rover_x < obs_right_edge + 5:
                    track_half_width = float(latest.get("track", {}).get("half_width", 60.0))
                    space_on_left = obs_left_edge - (-track_half_width)
                    space_on_right = track_half_width - obs_right_edge
                    if space_on_right > space_on_left:
                        send_can_frame(net, CAN_ID_STEER, (100).to_bytes(1, byteorder='little', signed=True))
                    else:
                        send_can_frame(net, CAN_ID_STEER, (-100).to_bytes(1, byteorder='little', signed=True))
                else:
                    send_can_frame(net, CAN_ID_STEER, (0).to_bytes(1, byteorder='little', signed=True))
            else:
                send_can_frame(net, CAN_ID_STEER, (0).to_bytes(1, byteorder='little', signed=True))
        ...
        for e in pygame.event.get():
            ...
            if e.type == pygame.KEYDOWN:
                if e.key in (pygame.K_ESCAPE, pygame.K_q):
                    ...
                    send_can_frame(net, CAN_ID_THROTTLE, (0).to_bytes(1, byteorder='little', signed=True))
                elif e.key == pygame.K_LEFT:
                    send_can_frame(net, CAN_ID_STEER, (-100).to_bytes(1, byteorder='little', signed=True))
                elif e.key == pygame.K_RIGHT:
                    send_can_frame(net, CAN_ID_STEER, (100).to_bytes(1, byteorder='little', signed=True))
                elif e.key == pygame.K_b:
                    send_can_frame(net, CAN_ID_BRAKE)
                elif e.key == pygame.K_s:
                    send_can_frame(net, CAN_ID_START_RACE)
                elif e.key == pygame.K_r:
                    send_can_frame(net, CAN_ID_RESET)

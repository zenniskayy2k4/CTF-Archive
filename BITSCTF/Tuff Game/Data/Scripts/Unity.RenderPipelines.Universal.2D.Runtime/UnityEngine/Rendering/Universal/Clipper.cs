...
namespace UnityEngine.Rendering.Universal
{
	internal class Clipper : ClipperBase
	{
		...
		private void SetWindingCount(TEdge edge)
		{
			...
			if (prevInAEL == null)
			{
				PolyFillTypes polyFillTypes = ((edge.PolyTyp == PolyTypes.ptSubject) ? m_SubjFillType : m_ClipFillType);
				if (edge.WindDelta == 0)
				{
		...
		private void IntersectEdges(TEdge e1, TEdge e2, IntPoint pt)
		{
			...
			if (e1.WindDelta == 0 || e2.WindDelta == 0)
			{
				...
				if (e1.PolyTyp == e2.PolyTyp && e1.WindDelta != e2.WindDelta && m_ClipType == ClipTypes.ctUnion)
				{
					if (e1.WindDelta == 0)
					{
						if (flag2)
				...
				else
				{
					...
					if (e1.WindDelta == 0 && Math.Abs(e2.WindCnt) == 1 && (m_ClipType != ClipTypes.ctUnion || e2.WindCnt2 == 0))
					{
						AddOutPt(e1, pt);
						if (flag)
						{
					...
					else if (e2.WindDelta == 0 && Math.Abs(e1.WindCnt) == 1 && (m_ClipType != ClipTypes.ctUnion || e1.WindCnt2 == 0))
					{
						AddOutPt(e2, pt);
						if (flag2)
						{
			...
			{
				PolyFillTypes.pftPositive => e1.WindCnt, 
				PolyFillTypes.pftNegative => -e1.WindCnt, 
				_ => Math.Abs(e1.WindCnt), 
			};
			int num2 = polyFillTypes3 switch
			{
				PolyFillTypes.pftPositive => e2.WindCnt, 
				PolyFillTypes.pftNegative => -e2.WindCnt, 
				_ => Math.Abs(e2.WindCnt), 
			};
			if (flag && flag2)
			{
				if ((num != 0 && num != 1) || (num2 != 0 && num2 != 1) || (e1.PolyTyp != e2.PolyTyp && m_ClipType != ClipTypes.ctXor))
				{
			...
			else
			{
				...
				{
					PolyFillTypes.pftPositive => e1.WindCnt2, 
					PolyFillTypes.pftNegative => -e1.WindCnt2, 
					_ => Math.Abs(e1.WindCnt2), 
				};
				long num4 = polyFillTypes4 switch
				{
					PolyFillTypes.pftPositive => e2.WindCnt2, 
					PolyFillTypes.pftNegative => -e2.WindCnt2, 
				...
				else if (num == 1 && num2 == 1)
				{
					...
					case ClipTypes.ctUnion:
						...
						break;
					case ClipTypes.ctDifference:
						if ((e1.PolyTyp == PolyTypes.ptClip && num3 > 0 && num4 > 0) || (e1.PolyTyp == PolyTypes.ptSubject && num3 <= 0 && num4 <= 0))
						{
							AddLocalMinPoly(e1, e2, pt);
						}
						break;
		...
		private void ProcessHorizontal(TEdge horzEdge)
		{
			...
			while (true)
			{
				...
				while (tEdge3 != null)
				{
					...
					if ((Dir == Directions.dLeftToRight && tEdge3.Curr.X > Right) || (Dir == Directions.dRightToLeft && tEdge3.Curr.X < Left) || (tEdge3.Curr.X == horzEdge.Top.X && horzEdge.NextInLML != null && tEdge3.Dx < horzEdge.NextInLML.Dx))
					{
						break;
					}
					if (horzEdge.OutIdx >= 0 && !flag)
		...
		{
			Directions directions = ((op1.Pt.X <= op1b.Pt.X) ? Directions.dLeftToRight : Directions.dRightToLeft);
			Directions directions2 = ((op2.Pt.X <= op2b.Pt.X) ? Directions.dLeftToRight : Directions.dRightToLeft);
		...
		public static List<List<IntPoint>> SimplifyPolygon(List<IntPoint> poly, PolyFillTypes fillType = PolyFillTypes.pftEvenOdd)
		{
			...
			clipper.AddPath(poly, PolyTypes.ptSubject, Closed: true);
		...
		public static List<List<IntPoint>> SimplifyPolygons(List<List<IntPoint>> polys, PolyFillTypes fillType = PolyFillTypes.pftEvenOdd)
		{
			...
			Clipper clipper = new Clipper();
			clipper.StrictlySimple = true;
			clipper.AddPaths(polys, PolyTypes.ptSubject, closed: true);
			clipper.Execute(ClipTypes.ctUnion, list, fillType, fillType);
			return list;
		}
		...
		public static List<List<IntPoint>> MinkowskiSum(List<IntPoint> pattern, List<IntPoint> path, bool pathIsClosed)
		{
			...
			Clipper clipper = new Clipper();
			clipper.AddPaths(list, PolyTypes.ptSubject, closed: true);
		...
		public static List<List<IntPoint>> MinkowskiSum(List<IntPoint> pattern, List<List<IntPoint>> paths, bool pathIsClosed)
		{
			...
			for (int i = 0; i < paths.Count; i++)
			{
				...
				clipper.AddPaths(ppg, PolyTypes.ptSubject, closed: true);
				if (pathIsClosed)
				{
					List<IntPoint> pg = TranslatePath(paths[i], pattern[0]);
					clipper.AddPath(pg, PolyTypes.ptClip, Closed: true);
				}
			}
			clipper.Execute(ClipTypes.ctUnion, list, PolyFillTypes.pftNonZero, PolyFillTypes.pftNonZero);
			return list;

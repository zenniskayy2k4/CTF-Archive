...
def solve_challenge(c_py, e_py, n_py):
    ...
    for i in range(m + 1):
        for j in range(m - i + 1):
            g = x^i * pol^j * e^(m-j)
            shifts.append(g)
            for k in range(g.degree(x) + 1):
                for l in range(g.degree(y) + 1):
    ...
    P_QQ.<x_q, y_q> = PolynomialRing(QQ)
    ...
    for i in range(dim):
        ...
        q1 += B_LLL[0][i] / mon(X, Y) * mon(x_q, y_q)
        q2 += B_LLL[1][i] / mon(X, Y) * mon(x_q, y_q)
    # =================================
    ...
    for k_sol, _ in k_roots:
        ...
        for s_sol, _ in s_roots:
            ...
            phi_candidate = n - s
            if phi_candidate > 0 and (1 + k * phi_candidate) % e == 0:

...
_sage_const_16 = Integer(16); _sage_const_0 = Integer(0); _sage_const_1 = Integer(1)
...
def pkcs7_unpad(data: bytes, block_size: int = _sage_const_16 ) -> bytes:
    if not data or (len(data) % block_size) != _sage_const_0 :
        raise ValueError("Invalid padded data length")
    pad_len = data[-_sage_const_1 ]
    if pad_len < _sage_const_1  or pad_len > block_size:
...
def parse_output_txt(text: str) -> tuple[str, str, str]:
    ...
    iv_hex = iv_m.group(_sage_const_1 )
    enc_hex = enc_m.group(_sage_const_1 )

    # Generator does: key = sha256(str(sk).encode()).digest()
    # output.txt line is: 'sk =  ' + str(sk)
    sk_str = sk_m.group(_sage_const_1 )
    first_paren = sk_str.find("(")
    if first_paren == -_sage_const_1 :
...
def main() -> None:
    ...
    pt = pkcs7_unpad(pt, _sage_const_16 )
